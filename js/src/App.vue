<template>
  <div class="wrap">
    <div>{{ playStates === 3 ? `正在播放第${curPlay + 1}首` : playStates === 2 ? `第${curPlay + 1}首暂停中` :"无音乐播放" }}</div>
    <br/>
    <button @click="changePlayState">{{ playStates === 1 ?"播放" : playStates === 2 ? "播放" : "暂停" }}</button>
    <br/>
    <button @click="next">下一首</button>
    <br/>
    <button @click="prev">上一首</button>
    <br/>
    <button @click="changePlayMode">{{ playMode === 0 ? "列表循环" : playMode === 1 ? "随机播放" : "单曲循环" }}</button>
    <br/>
...
function prev() {
  curPlay.value = curPlay.value - 1 === -1 ? music.length - 1 : curPlay.value - 1;
  play();
}
